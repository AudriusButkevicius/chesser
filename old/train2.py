...
class PathRecordIterator(Iterator):
    ...
    def to_record(cls, data: bytes):
        ...
        #score = (np.clip(record['score'].astype(model_dtype), -15, 15) + 15) / 30
...
class EvaluationModel(pl.LightningModule):
    def __init__(
        ...
        self.conv3 = nn.Conv2d(in_channels=256, out_channels=256, kernel_size=3, dtype=torch_dtype)
        self.pool = nn.MaxPool2d(2)

        self.activation = nn.ReLU

        layers: List[Tuple[str, any]] = [
            #(f'linear-entry', nn.Linear((256 * 2 * 2) + (12 * 8 * 8) + 3, hidden_layer_width, dtype=torch_dtype)),
            (f'linear-entry', nn.Linear((12 * 8 * 8) + 3, hidden_layer_width, dtype=torch_dtype, bias=False)),
            (f'activation-entry', self.activation())
        ]
        ...
        for i in range(hidden_layers):
            ...
            layers.append(
                (f'activation-{i}', self.activation())
            )
    ...
    def forward(self, board, features):
        ...
        # conv_board = F.relu(self.conv1(board))
        # conv_board = F.relu(self.conv2(conv_board))
        # conv_board = F.relu(self.conv3(conv_board))
        # print(conv_board.shape)
        # BOOM
...
def main():
    ...
    lr = 1e-4
    model = EvaluationModel(train_paths, val_paths, batch_size=1024, learning_rate=lr, hidden_layers=6, hidden_layer_width=2048)
    callbacks = [
        StochasticWeightAveraging(swa_lrs=lr, device=None),
        EarlyStopping(monitor="val_loss"),
    ]
    accumulate_grad_batches = 7
    trainer = pl.Trainer(accelerator="gpu", max_epochs=2000, callbacks=callbacks, accumulate_grad_batches=accumulate_grad_batches, log_every_n_steps=50)

    trainer.fit(model, ckpt_path=list(pathlib.Path(
        r"../lightning_logs/version_31/checkpoints").rglob("*.ckpt"))[0])
