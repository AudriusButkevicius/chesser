...
class PlayThread(threading.Thread):
    def __init__(self, window: MainWindow, use_model: bool, depth: int):
        ...
        if use_model:
            self.model = EvaluationModel.load_from_checkpoint(
                list(
                    pathlib.Path(r"../lightning_logs/version_31/checkpoints").rglob("*.ckpt")
                )[0]
            )
        ...
        with engine:
            ...
            try:
                while not board.is_game_over():
                    ...
                    my_move_score1 = self.to_score(self.model_score(my_actual_score))
                    ...
                    opponent_move_score1 = self.to_score(self.model_score(my_score))
    ...
    def evaluate_boards_model(self, *boards: chess.Board):
        ...
        one_hot_boards = np.zeros((batch_size, 12, 64), dtype=np.uint8)
        other_features = np.zeros((batch_size, 1, 3), dtype=np.float32)
        for n, board in enumerate(boards):
            ...
            for i, piece in enumerate(piece_types):
                ...
                one_hot_boards[n][i] = np.unpackbits((piece & white).view(np.uint8)).astype(np.uint8)
                one_hot_boards[n][i + 6] = np.unpackbits((piece & ~white).view(np.uint8)).astype(np.uint8)
            ...
            other_features[n] = np.array([other_feature_list]).astype(np.float32)

        one_hot_boards = one_hot_boards.astype(np.float32)
        one_hot_boards.shape = (batch_size, 12, 8, 8)
        other_features.shape = (batch_size, 3)
        ...
        with torch.no_grad():
            return self.model(one_hot_boards_tensor, other_features_tensor)
        ...
        for square in chess.SQUARES:
            ...
            if piece is not None and piece.piece_type == chess.PAWN:
                if piece.color == chess.WHITE:
                    pawn_structure_count += 1 if square in [chess.A2, chess.B2, chess.C2, chess.D2, chess.E2, chess.F2, chess.G2, chess.H2] else 0
                else:
                    pawn_structure_count -= 1 if square in [chess.A7, chess.B7, chess.C7, chess.D7, chess.E7, chess.F7, chess.G7, chess.H7] else 0
...
if __name__ == "__main__":
    ...
    window = MainWindow()
    window.show()
    play_thread = PlayThread(window, use_model=True, depth=3)
    app.exec()
